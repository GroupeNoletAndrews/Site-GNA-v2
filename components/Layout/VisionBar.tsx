    ...
    <div className="hidden md:flex w-full h-10 md:h-20 items-center bg-white border-t border-slate-200 z-50 relative shadow-sm transition-all duration-300">
      <div className="w-full h-full flex overflow-x-auto no-scrollbar md:justify-center">
        {VISION_ITEMS.map((vision, index) => {
          // Utiliser les traductions pour chaque item de vision
          const visionItems = t('vision.items') as any[];
          const visionItem = visionItems && Array.isArray(visionItems) && visionItems[index] ? visionItems[index] : vision;
          const label = visionItem?.label || vision.label;
          const description = visionItem?.description || vision.description;
