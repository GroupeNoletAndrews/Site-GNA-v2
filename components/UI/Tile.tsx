...
const Tile: React.FC<TileProps> = ({ item, isActive, isAnyActive, isIntroMode = false, isIntroExiting = false, introAnimationFinished = false, isPortrait = false, collapsedIndex, onActivate, onClose, onBackToIntro, hoveredId, setHoveredId }) => {
  ...
  if (isPortrait) {
    if (item.id === "hero") {
      ...
      const navClass =
        PORTRAIT_NAV_CONFIG[item.id] || "col-span-1 aspect-square";
    ...
    if (item.id === "hero") {
      gridClasses =
        "col-span-1 md:col-start-1 md:col-span-1 md:row-start-1 md:row-span-6 z-30 cursor-pointer";
    } else {
      ...
      if (leftIndex !== -1) {
        ...
        if (targetRow) {
          const spanClass =
            item.id === "contact" ? "md:row-span-2" : "md:row-span-1";
    ...
    const rowSpanClass =
      {
        ...
        5: "row-span-1 md:row-span-5",
        6: "row-span-1 md:row-span-6",
  ...
  if (isAnyActive && !isPortrait) {
    if (item.id === "hero") {
      borderClass = `border-r ${borderColor}`;
    } else if (isLeftSidebarItem) {
      borderClass = `border-b ${borderColor}`;
    } else {
      borderClass = `border-b border-l ${borderColor}`;
    }
  } else if (isIntroMode) {
    if (item.id === "hero") {
      borderClass = `border-r ${borderColor}`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`absolute inset-0 ${backgroundClass} ${borderClass} transition-all duration-500`}
        />
        ...
        {item.imageUrl && !isButtonMode && !isHero && !isPortrait && (
          <motion.img
            ...
            src={item.imageUrl}
            className="absolute inset-0 w-full h-full object-cover mix-blend-overlay grayscale transition-opacity duration-500"
          />
        )}
        ...
        <motion.div
          ...
          animate={{
            scaleX:
              !isAnyActive && !isIntroMode && !isPortrait ? inverseScaleX : 1,
          }}
          style={{ originX: 0 }}
          transition={mainTransition}
        >
            ...
            <div className="w-full h-full relative z-10">
              {isPortrait ? (
                ...
                <div className="w-full h-full relative p-6 flex flex-col justify-between">
                        ...
                        >
                          ...
                          <motion.button
                            ...
                            transition={{
                              opacity: {
                                duration: 0,
                                delay: isIntroExiting ? 0 : 2,
                              },
                            }}
                            className={`flex flex-col items-center gap-2 animate-bounce cursor-pointer ${isIntroExiting ? "pointer-events-none" : ""}`}
                          >
                  ...
                  {/* BOTTOM: ACCUEIL BUTTON */}
                  <div className="flex justify-center">
                    {!isIntroMode && onBackToIntro && (
                      <motion.button
                ...
                <div className="w-full h-full flex flex-col">
                  ...
                  <div
                    className={`flex-grow flex flex-col items-center w-full transition-all duration-500 ease-in-out px-4
                                ${
                                  isIntroMode
                  ...
                  <AnimatePresence>
                    {!isIntroMode && !isAnyActive && (
                      <motion.div
                        key="hero-button-description-container"
                        ...
                        className="absolute bottom-0 left-4 right-4 xl:left-6 xl:right-6 pointer-events-none flex flex-col justify-end gap-6 md:pb-16 lg:pb-20 xl:pb-24 2xl:pb-28"
                      ...
                      >
                        ...
                        {/* DESCRIPTION (SUBTEXT) */}
                        <div className="flex justify-center">
                          <div className="border-l-2 border-white/40 pl-4 hidden md:block pointer-events-none">
                            <p className="font-normal text-xs md:text-sm lg:text-base xl:text-lg leading-snug text-slate-100 max-w-[220px] md:max-w-[240px] xl:max-w-[320px] drop-shadow-md">
                              {item.description}
                            </p>
                          </div>
                        </div>
            ...
            <AnimatePresence mode="wait">
              ...
              {isButtonMode || isIntroMode || isPortrait ? (
                <motion.div
                  key="compact-mode"
                  initial={
                    isPortrait ? { opacity: 1 } : { opacity: 0, scale: 0.8 }
                  }
                  animate={{
                    opacity: isIntroExiting ? 0 : 1,
                    scale: 1,
                ...
                >
                  <div
                    className={`${isActive ? "opacity-100 scale-110" : "opacity-70 group-hover:opacity-100"} transition-transform duration-300 relative z-20`}
                  >
                    {item.icon && (
                      <item.icon
                        className={
                ...
                <motion.div
                  ...
                  animate={
                    isExiting
                      ...
                      : {
                          opacity:
                            !introAnimationFinished && !isIntroMode ? 0 : 1,
                          transition: {
                  ...
                  className={`w-full h-full flex flex-col justify-between ${item.textClass} relative overflow-hidden
                                ...
                                ${
                                  isRightSide
                                    ? "md:py-5 md:pr-5 md:pl-10 md:pb-8 xl:py-8 xl:pr-8 xl:pl-12 xl:pb-10"
                                    : "md:p-5 md:pb-8 xl:p-8 xl:pb-10"
                                }
                            `}
                >
                  ...
                  <div className="flex flex-row items-center gap-3 w-full pr-6">
                    <div className="shrink-0">
                      {item.icon && (
                        <item.icon className="w-4 h-4 md:w-5 md:h-5 lg:w-6 lg:h-6 xl:w-8 xl:h-8" />
                      )}
                    </div>
